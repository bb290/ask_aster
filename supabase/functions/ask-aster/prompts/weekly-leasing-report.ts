// AUTO-GENERATED from skills/weekly-leasing-report/SKILL.md by scripts/sync-prompts.ts
// Do not edit this file by hand. Edit the SKILL.md, then re-run sync-prompts.

export const weeklyLeasingReportPrompt = {
  name: "weekly-leasing-report",
  description: "Compose the weekly leasing activity email a leasing agent sends to a property owner. Aster pulls list date and last week's report from Asana for context, then asks the agent one batched question for this week's activity numbers, any prospect or agent feedback, and optionally up to two comparable Zillow listings. Recommends activity-boosting strategies if numbers are soft. Outputs the email formatted in Sagareus voice for the agent to copy-paste into Gmail. If Asana MCP is connected, also posts a copy of the email to the property's \"Send weekly activity report\" sub-task. Use when an agent says \"weekly leasing report,\" \"weekly update for [property],\" or invokes /weekly-leasing-report. Sagareus standard send-day at the agent level is Tuesday. (The customer-facing SOP says Wednesday as a safeguard so the manager has a buffer day to send it if an agent misses Tuesday. Agents should always aim for Tuesday.)",
  content: `# Weekly Leasing Activity Report — agent composer
...
- **List Date** — stored as a custom field named "List Date." If missing, ask the agent.
- **Zillow Link** — stored as a custom field named "Zillow Link." If missing, ask the agent.
- **Redfin Link** — stored as a custom field named "Redfin Link." If missing, ask the agent.
- **Sagareus Link** — stored as a custom field named "Sagareus Link." If missing, ask the agent.
- **Days on market** — calculated, not stored. \`today() - list_date\`.

All three listing-URL fields are required because the email's "Listing links" section always cites Zillow, Redfin, and Sagareus.

(Owner email is intentionally not pulled. The email output is plain text and the agent fills in the recipient when they paste it into Gmail.)
...
After everything is gathered, compose. Show it to the agent for review before any writes.

The output is the email body only. No subject line. The agent will write their own subject when pasting into Gmail. The body ends with \`Thanks,\` (or \`Best,\`) and stops there. Do not add an agent name or "Sagareus" tag, the agent's Gmail signature handles that.

\`\`\`
Hi [Owner First Name],

Quick update on [Property Address] for the week ending [Date].
...
[Optional: prospect feedback or agent observations from the batched answer.]

[If at least one comp was provided, include this section. Omit entirely if zero comps:]
Market context, comparable rentals:
  1. [Address or neighborhood], [bed]bd/[bath]ba, [sqft] sqft, $[rent]/mo, [days] days on market.
     [Zillow link]
  [2. only if a second comp was provided]

Listing update:
[Always included. Pick the form that matches activity:]

  [If activity is adequate (>10 inquiries OR >5 showings):]
  Holding price another week. Activity is healthy.

  [If activity is soft (<= 10 inquiries AND <= 5 showings):]

  We're dropping the asking rent by $[delta] to drive activity.

  [Only include the next block if the agent picked at least one additional strategy:]
  We also recommend the following moves, pending your approval:
    • [Strategy in plain language, e.g. "Decreasing the security deposit to half of one month's rent."]
    • [Strategy in plain language, e.g. "Adding a $500 move-in rent credit at lease signing."]
    • [Strategy in plain language, e.g. "Removing the last month's rent requirement."]
    • [Strategy in plain language, e.g. "Increasing the pet weight limit to 60 lbs."]

  Reply with which of these you'd like us to implement.

  [If conversion problem (>10 inquiries AND <5 showings):]
  Plenty of inquiries this week but showings didn't keep pace. Reviewing how we
  respond to leads to make sure prospects are getting on the calendar quickly.
  [Plus, only if the agent picked additional strategies:]
  We also recommend the following moves, pending your approval:
    • [Each agent-picked strategy in plain language.]
  Reply with which of these you'd like us to implement.

Next update: Tuesday, [next Tuesday's date].
...
- No corporate filler.
- No subject line in the output. The agent writes their own when pasting.
- The "Listing links" section at the bottom (Zillow, Redfin, Sagareus) is **always present**. All three URLs come from the LU/TP sub-task custom fields. If any URL is missing in Asana, the batched-question prompt asks the agent to paste it; that override gets used in the email.
...
## Output the email (always)

Always output the email body in a clean copy-paste block. No subject line. The body starts with "Hi [Owner First Name]," and ends with "Thanks," (or "Best,"). See the full template below.

The agent pastes the body into Gmail, writes their own subject line, fills in the owner's first name and email recipient, and sends. Aster does not create a Gmail draft and does not need the owner's email address.

## Asana write (post the email body to the sub-task)

After the agent approves the draft:

1. Find the "Send weekly activity report" sub-task GID (already located during lookup, reuse it).
2. Post the approved email body as a story (comment) using \`mcp__asana__asana_create_task_story\`.
3. Tell the agent: "Posted a copy to [Property] → LU → Send weekly activity report. Mark the sub-task complete after you've sent the email."
4. Don't auto-complete. The agent does that after sending.

If any step fails, do not block the agent. Output a one-line note ("Couldn't post to Asana automatically (reason)") and re-output the email body for manual paste.
...
- Sending the email. Aster only outputs text.
- Creating a Gmail draft. Aster does not address or compose into Gmail. Agent pastes manually.
- Pulling the owner's email address from Asana. Bug-prone, dropped in v0.
- Auto-completing the Asana sub-task. The agent does that.
