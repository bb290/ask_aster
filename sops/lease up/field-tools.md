...
* **07-24** — Move Out v3, built to the ideal flow: Step 1 Locks + Safety (lockbox code changed + recorded to 1Password, mailbox keys, exterior locks, smoke/CO — confirm each; new keybox location + code) → Step 2 Turn Everything On → Step 3 Video Walkthrough (direct upload from the phone, required) → Step 4 Room-by-Room Inspection (only note what is wrong, everything else is assumed OK; each issue = trade type + note + required photo) → Step 5 Cleanliness (grade + Odor + Stains). The report, PDF, and Turn Over work order all group findings BY TRADE — "Flooring: Living Room, Kitchen" — so scope hands to vendors by trade. Rooms and trade types are editable in the page editor.
