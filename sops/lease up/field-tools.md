...
* **07-24** — Move Out v2: every area now lists its items (Blinds, Flooring, Walls...) with OK / Document Issue per item and an All OK button per section — the item list is editable in the page editor. Keybox and video moved to the top, and the video is now a direct upload button with a progress percent (Drive-link fallback if the upload fails).
